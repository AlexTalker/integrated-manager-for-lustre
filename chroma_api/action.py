...
class ActionResource(Resource):
    ...
    display_group = fields.CharField(attribute="display_group", readonly=True)
    display_order = fields.CharField(attribute="display_order", readonly=True)
    ...
    def get_object_list(self, request):
        ids = map(lambda x: x.split(":"), request.GET.getlist(ID_TYPES))
        ...
        for _, id in ids:
            obj_transitions_states_and_verbs = computed_transitions[id]
            obj_jobs = computed_jobs[id]

            available_actions = sorted(
                obj_transitions_states_and_verbs + obj_jobs, key=lambda action: action["display_order"]
            )
            actions += map(lambda x: Action(**x), available_actions)
